...
const schema = Yup.object().shape({
    ...
    password: Yup.string().min(3, 'La contraseña debe tener al menos 3 caracteres').required('La contraseña es obligatoria'),
})
        ...
        <div className='w-6/12 mx-auto my-14 border-2 border-slate-300 rounded-xl'>
            ...
            <form onSubmit={ handleSubmit } className='p-14'>
                ...
                <label className="input input-bordered flex items-center gap-2 m-4">
                    ...
                    <input type="text" className="grow" placeholder="Usuario" name='username' onChange={handleChange}/>
                </label>
                {errors.username && touched.username && <div role="alert" className="alert alert-error">
                    <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    <span>{errors.username}</span>
                    </div>}
                <label className="input input-bordered flex items-center gap-2 m-4">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 opacity-70"><path fillRule="evenodd" d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z" clipRule="evenodd" /></svg>
                    <input type="password" className="grow" placeholder="Contraseña" name='password' onChange={handleChange}/>
                </label>
                {errors.password && touched.password && <div role="alert" className="alert alert-error">
                    <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    <span>{errors.password}</span>
                    </div>}
                <button className="btn m-4" type='submit' onSubmit={submitForm}>Agregar usuario</button>
            </form>
        </div>
